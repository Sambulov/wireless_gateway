...
#define API_CALL_STATUS_OK                          0x00000000
#define API_CALL_STATUS_CANCELED                    0x00000001
#define API_CALL_STATUS_BUSY                        0x00000002
#define API_CALL_STATUS_WR_ERROR                    0x00000003
#define API_CALL_STATUS_ACCESS_ERROR                0x00000004
...
/*!
	...
	@param[in] eReason call reason 
	  API_CALL_PRIMARY for the first time, 
	  API_CALL_SECONDARY next calls, 
	  API_CALL_BREAK in case if connection terminated
	@return 0 if prolonged call, else - complete
*/
typedef uint8_t (*ApiHandler_t)(void *pxApiCall, void **ppxContext, ApiCallReason_t eReason, uint8_t *pucData, uint32_t ulDataLen);
